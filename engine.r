...
calTeamPastStats <- function(date,newtable,HorA){
  a<-list()
...
simulation <- function(date,HomeTeam,AwayTeam){
  ...
  eFG <- rnorm(100,mean = statsH$normPar_eFG[1], sd = sqrt(statsH$normPar_eFG[2])) - rnorm(100,mean = statsA$normPar_eFG[1], sd = sqrt(statsA$normPar_eFG[2]))
  TOV <- rnorm(100,mean = statsH$normPar_TOV[1], sd = sqrt(statsH$normPar_TOV[2])) - rnorm(100,mean = statsA$normPar_TOV[1], sd = sqrt(statsA$normPar_TOV[2]))
  ORB <- rnorm(100,mean = statsH$normPar_ORB[1], sd = sqrt(statsH$normPar_ORB[2])) - rnorm(100,mean = statsA$normPar_ORB[1], sd = sqrt(statsA$normPar_ORB[2]))
  FT <- rnorm(100,mean = statsH$normPar_FT[1], sd = sqrt(statsH$normPar_FT[2])) - rnorm(100,mean = statsA$normPar_FT[1], sd = sqrt(statsA$normPar_FT[2]))
  pred_scorediff <- param_list[1] + param_list[2] * eFG + param_list[3]* TOV + param_list[4]* ORB + param_list[5] * FT
  winRatio_H <- sum(pred_scorediff>0)/100
  return(winRatio_H)
}
